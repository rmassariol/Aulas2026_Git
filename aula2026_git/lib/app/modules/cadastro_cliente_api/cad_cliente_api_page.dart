import 'package:aula2026_git/app/modules/cadastro_cliente_api/api_helper.dart';
import 'package:flutter/material.dart';

class CadClienteApiPage extends StatefulWidget {
  const CadClienteApiPage({super.key});

  @override
  State<CadClienteApiPage> createState() => _CadClienteApiPageState();
}

class _CadClienteApiPageState extends State<CadClienteApiPage> {
  ...
  Future<void> _carregarClientes() async {
    ...
    try {
      final lista = await _apiHelper.listarClientes();
  ...
  void _editarCliente(Map<String, dynamic> cliente) {
    setState(() {
      _codigoController.text = cliente['_id'].toString();
  ...
  Future<void> _salvarCliente() async {
    ...
    try {
      ...
      if (codigoTexto.isNotEmpty) {
        await _apiHelper.atualizarClientes(codigoTexto, cliente);

        setState(() {
          _mensagem = 'Cliente atualizado com sucesso!';
        });
      } else {
        final codigoGerado = await _apiHelper.inserirClientes(cliente);
  ...
  Future<void> _deletarCliente(String codigo, String nome) async {
    ...
    try {
      await _apiHelper.deletarClientes(codigo);
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          child: ListTile(
                            ...
                            subtitle: Text('Código: ${cliente['_id']}'),
                            trailing: Row(
